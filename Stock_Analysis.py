...
# --- Example NSE stock list (trimmed for demo, you can keep full list) ---
...
if dropdown_choice == "Enter Manually":
    symbol = st.sidebar.text_input("Enter Stock Symbol", "").strip().upper()
else:
    symbol = dropdown_choice

# --- Quick Date Range Slider ---
range_labels = ["Manual", "1W", "1M", "6M", "1Y"]

date_range_option = st.sidebar.select_slider(
    "Select Day Range",
...
if date_range_option == "1W":
    ...
    from_date, to_date = today - pd.DateOffset(years=1), today
# if "Custom" → manual input is used
...
with col2:
    fetch_btn = st.button("Fetch Chart")

if clear_btn:
    st.cache_data.clear()
    st.success("Cache cleared! Please re-run with a new stock/date.")
...
if fetch_btn and symbol:
    ...
    num_cols = ["PrevClose", "OpenPrice", "HighPrice", "LowPrice",
    ...
    df["%PriceChange"] = df["ClosePrice"].pct_change() * 100
    df["%PriceChange"] = df["%PriceChange"].fillna(0)

    df["%DlyQttoTradedQty"] = df["%DlyQttoTradedQty"].astype(str).replace('-', pd.NA).str.rstrip('%')
    df["%DlyQttoTradedQty"] = pd.to_numeric(df["%DlyQttoTradedQty"], errors='coerce')
    df["%DlyQttoTradedQty"] = df["%DlyQttoTradedQty"].ffill().fillna(0).clip(0, 100)

    df["DeliverableQty"] = pd.to_numeric(df["DeliverableQty"], errors='coerce')
    df["TotalTradedQuantity"] = pd.to_numeric(df["TotalTradedQuantity"], errors='coerce')
    df["No.ofTrades"] = pd.to_numeric(df["No.ofTrades"], errors='coerce')
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            name="Price",
            hoverinfo="text",
            text=candlestick_text
        ),
        row=1, col=1
    )

    # 2. Deliverable vs Non-Deliverable
    fig.add_trace(
        go.Bar(
            x=df["Date"],
            y=df["DeliverableQty"],
    ...
    fig.add_trace(
        go.Bar(
            x=df["Date"],
            y=df["TotalTradedQuantity"] - df["DeliverableQty"],
            name="Intraday Qty",
            marker_color="red",
    ...
    fig.update_yaxes(title_text="No. of Trades", row=3, col=1, secondary_y=False,
                     range=[0, max(df["No.ofTrades"]) * 1.2])
    fig.update_yaxes(title_text="% Deliverable", row=3, col=1, secondary_y=True,
